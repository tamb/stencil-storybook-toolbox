...
export function extractStencilInfo(element: HTMLElement): StencilComponentInfo | null {
  ...
  if (constructor.observedAttributes) {
    info.props = constructor.observedAttributes.map((attr: string) => ({
  ...
  if (constructor.componentOnReady) {
    // This is a Stencil component - access its internal metadata
    const metadata = (constructor as any).__stencil_metadata__;
    if (metadata) {
      if (metadata.properties) {
        info.props = Object.entries(metadata.properties).map(([name, prop]: [string, any]) => ({
          name,
      ...
      if (metadata.states) {
        info.state = Object.entries(metadata.states).map(([name, state]: [string, any]) => ({
          name,
      ...
      if (metadata.methods) {
        info.methods = Object.entries(metadata.methods).map(([name, method]: [string, any]) => ({
          name,
      ...
      if (metadata.events) {
        info.events = Object.entries(metadata.events).map(([name, event]: [string, any]) => ({
          name,
          detail: event.detail,
      ...
      if (metadata.listeners) {
        info.listeners = metadata.listeners.map((listener: any) => ({
 ...
 * ```
 */
export function getStencilElement(context: any): HTMLElement | null {
  ...
  const definedElements = Array.from(storyElement.querySelectorAll('*')).find(
    (el: Element) => el.tagName.includes('-')
