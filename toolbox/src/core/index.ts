 ...
 */
const DEFAULT_CACHE_DIRS = ['node_modules/.vite-storybook', '.storybook-cache'];
 ...
 * @internal
 */
export function createStencilConfig(baseConfig: UserConfig, options: Required<UseStencilOptions>): UserConfig {
  // Normalize packageName to always be an array
  const packageNames = Array.isArray(options.packageName) ? options.packageName : options.packageName ? [options.packageName] : [];
  ...
  return {
    ...
    server: {
      ...
      headers: options.disableCache
        ...
        : (baseConfig.server?.headers ?? undefined),
    },
    optimizeDeps: {
      ...baseConfig.optimizeDeps,
      exclude: [...(baseConfig.optimizeDeps?.exclude || []), ...packageNames],
 ...
 * @internal
 */
export function createViteFinalWrapper(viteFinal?: ViteFinalConfig, options?: UseStencilOptions) {
  ...
  // Ensure the cache directory exists after clearing (or if it doesn't exist)
  ensureDirectoryExists(mergedOptions.cacheDir);
