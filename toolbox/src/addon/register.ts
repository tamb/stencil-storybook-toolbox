...
import { registerStencilAddon, type AddonAPI } from './manager';
import type { StencilComponentInfo } from './types';

// Storybook will provide the API
declare const addons: {
  register: (id: string, callback: (api: AddonAPI) => void) => void;
  getChannel: () => {
...
if (typeof addons !== 'undefined') {
  addons.register(ADDON_ID, (api: AddonAPI) => {
    // Register the panel
    api.add(PANEL_ID, {
      type: 'panel',
      title: 'Stencil',
      match: ({ viewMode }: { viewMode: string }) => viewMode === 'story',
      render: ({ key }: { active?: boolean; key: string }) => {
        ...
        import('./panel-simple.js').then(({ renderStencilPanel }) => {
          ...
          const updatePanel = (info: unknown) => {
            renderStencilPanel(panelElement, info as StencilComponentInfo | null);
          };
